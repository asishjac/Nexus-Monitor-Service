...
def getFileList(final File baseDirectory) {
    def list = [];
    def pattern = "{" +
            "**/metadata.properties," +
            "**/*metrics.properties," +
    ...
    def dir = new File(baseDirectory.path);
...
blobStoreDirectories.each { blobStore ->
    ...
    File blobStoreFolder = new File(blobStore.absolutePath);
    BlobStatistics blobStat = new BlobStatistics();
    blobStat.availableSpace=blobStoreFolder.getFreeSpace();
    ...
    getFileList(blobStore).each { File propertiesFile ->
...
blobStatCollection.each() { blobStoreName, blobStat ->
    ...
    blobStoreName=blobStoreName.replace(".","_");
    blobStat.repositories.each{String repoName, RepoStatistics repoStatics ->
...
return groovy.json.JsonOutput.toJson(blobStatCollection
        .findAll {a, b -> b.repositories.size() > 0}
        .toSorted {a, b -> b.value.totalBlobStoreBytes <=> a.value.totalBlobStoreBytes});
